...
def execute_query_cpe(myuser,mypassword,myhost,database,cpe,score,date,csv_on,output_folder):
    ...
    try:
        ...
        print("Executing query\r\n")
        if date:
            cur.execute("SELECT cpe23uri, cve, base_score_3, vector_string_3, published_date FROM cvss_vs_cpes WHERE cpe23uri LIKE '%"+cpe+"%' AND (base_score_3 >= "+score+") AND (published_date >= '"+date+"'::date) ORDER BY published_date ASC, base_score_3 DESC, cpe23uri ASC")
            selected_cpe = cur.fetchall()
            print("CPE\t\t\t\t\t\t\tCVE\t\tCVSSv3.x CVSSv3.x Vector String \t\t\t Published Date")
            for r in selected_cpe:
                print(r[0],r[1],r[2],"\t",r[3],"\t",r[4])
                if csv_on:
                    cpes.append(r)
        else:
            cur.execute("SELECT cpe23uri, cve, base_score_3, vector_string_3, published_date FROM cvss_vs_cpes WHERE cpe23uri LIKE '%"+cpe+"%' AND (base_score_3 >= "+score+") ORDER BY published_date ASC, base_score_3 DESC, cpe23uri ASC")
            selected_cpe = cur.fetchall()
            print("CPE\t\t\t\t\t\t\tCVE\t\tCVSSv3.x CVSSv3.x Vector String \t\t\t")
            for r in selected_cpe:
        ...
        if csv_on:
            ...
            writer_cpe.writerow(["CPE","CVE","CVSSv3 Score","CVSSv3.x Vector String","Published Date"])
            for r in cpes:
                writer_cpe.writerow([r[0],r[1],r[2],r[3],r[4]])
            file_cpe.close()
            file_xlsx = pd.read_csv(output_folder+cpe+"_"+score+"_"+str(date)+".csv")
            file_xlsx.to_excel(output_folder+cpe+"_"+score+"_"+str(date)+".xlsx", index = None, header=True)
